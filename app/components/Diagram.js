...
function Diagram({code}) {
  ...
  useEffect(() => {
      ...
      const dataFromCode = 'newStack.push(12)newStack.push(23)newStack.pop()newStack.push(34)newStack.push(45)newStack.push(56)newStack.pop()newStack.pop()'
      // const dataFromCode = code
        
        let match;
        while ((match = pushRegex.exec(dataFromCode)) !== null) {
          pushOperations.push(parseInt(match[1]));
        }
        // get the type of the operation from the file
        let match1;
        while ((match1 = operationRegex.exec(dataFromCode)) !== null) {
